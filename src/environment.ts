...
export async function validateNodeConfig(runtime: IAgentRuntime): Promise<NodeConfig> {
  try {
    ...
    const config = {
      ...
      ELEVENLABS_XI_API_KEY:
        runtime.getSetting('ELEVENLABS_XI_API_KEY') || process.env.ELEVENLABS_XI_API_KEY,

      // Use character card settings first, fall back to env vars, then defaults
      ...(runtime.getSetting('ELEVENLABS_XI_API_KEY') && {
        ELEVENLABS_MODEL_ID:
          elevenlabs?.model || process.env.ELEVENLABS_MODEL_ID || 'eleven_monolingual_v1',
        ELEVENLABS_VOICE_ID: elevenlabs?.voiceId || process.env.ELEVENLABS_VOICE_ID,
        ELEVENLABS_VOICE_STABILITY:
          elevenlabs?.stability || process.env.ELEVENLABS_VOICE_STABILITY || '0.5',
        ELEVENLABS_VOICE_SIMILARITY_BOOST:
          elevenlabs?.similarityBoost || process.env.ELEVENLABS_VOICE_SIMILARITY_BOOST || '0.75',
        ELEVENLABS_VOICE_STYLE: elevenlabs?.style || process.env.ELEVENLABS_VOICE_STYLE || '0',
        ELEVENLABS_VOICE_USE_SPEAKER_BOOST:
          elevenlabs?.useSpeakerBoost || process.env.ELEVENLABS_VOICE_USE_SPEAKER_BOOST || 'true',
