...
export class TranscriptionService extends Service implements ITranscriptionService {
  ...
  private detectCuda() {
    ...
    if (platform === 'linux') {
      try {
        ...
        elizaLogger.log('CUDA detected. Transcription will use CUDA acceleration.');
      ...
      elizaLogger.log('CUDA not supported on this platform. Transcription will run on CPU.');
    }
  }
  ...
  private async transcribeWithDeepgram(audioBuffer: ArrayBuffer): Promise<string | null> {
    const buffer = Buffer.from(audioBuffer);
    const response = await this.deepgram.listen.prerecorded.transcribeFile(buffer, {
      model: 'nova-3',
      language: 'en',
      smart_format: true,
    });
