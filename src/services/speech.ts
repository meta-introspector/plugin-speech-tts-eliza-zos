...
async function getVoiceSettings(runtime: IAgentRuntime) {
  ...
  return {
    ...
    elevenlabsStability:
      elevenlabsSettings?.stability || runtime.getSetting('ELEVENLABS_VOICE_STABILITY') || '0.5',
...
async function generateVitsAudio(runtime: IAgentRuntime, text: string): Promise<Readable> {
  const { vitsVoice } = await getVoiceSettings(runtime);
  const { audio } = await Echogarden.synthesize(text, {
    engine: 'vits',
    voice: vitsVoice,
